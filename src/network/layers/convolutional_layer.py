...
class ConvolutionalLayer:
    ...
    def convolute(self, image):
        ...
        output_dim = int((input_dim - self.filter_size) /
            ...
            while img_y + self.filter_size <= input_dim:
                ...
                while img_x + self.filter_size <= input_dim:
                    output[filter, out_y, out_x] = \
                        np.sum(self.filters[filter] * image[filter, img_y:img_y+self.filter_size,
                               img_x:img_x+self.filter_size]) + self.bias_vector[filter]
    ...
    def update_parameters(self, batch_size, learning_rate, beta1, beta2, clip_threshold, iterations):
        ...
        for filter_i in range(len(self.filters)):
            # Gradient clipping
            self.gradient_filters[filter_i] = self._clip_gradient(
                self.gradient_filters[filter_i], clip_threshold)
            self.bias_gradients[filter_i] = self._clip_gradient(
                self.bias_gradients[filter_i], clip_threshold)
