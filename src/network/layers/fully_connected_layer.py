...
class FullyConnectedLayer:
    ...
    def process(self, images):
        ...
        for i in range(len(self.weight_matrixes)):
            ...
            flattened_image = np.dot(flattened_image, self.weight_matrixes[i]) + self.biases[i]
    ...
    def update_parameters(self, batch_size, learning_rate, beta1, beta2, iterations):
        ...
        for dense in range(self.num_dense_layers):
            ...
            weight_mhat = self.weight_mean_grads[dense] / (1 - beta1**(iterations+1))
            weight_vhat = self.weight_grad_variances[dense] / (1 - beta2**(iterations+1))
            # Update variable
            self.weight_matrixes[dense] -= learning_rate*weight_mhat / (np.sqrt(weight_vhat)+1e-7)
            ...
            bias_mhat = self.bias_mean_grads[dense] / (1 - beta1**(iterations+1))
            bias_vhat = self.bias_grad_variances[dense] / (1 - beta2**(iterations+1))
            self.biases[dense] -= learning_rate*bias_mhat / (np.sqrt(bias_vhat)+1e-7)
    ...
    def backpropagation(self, gradient_score, reg_strength):
        ...
        """
        self.received_inputs[-1] = np.array(self.received_inputs[-1]).reshape(1, -1)
        gradient_score = gradient_score.reshape(1, -1)

        self.gradient_weights[-1] += np.dot(self.received_inputs[-1].T, gradient_score)
        self.bias_gradients[-1] += np.sum(gradient_score)

        gradient_for_next_layer = np.dot(gradient_score, 
                                        self.weight_matrixes[-1].T)

        self.received_inputs[-2] = np.array(self.received_inputs[-2]).reshape(1, -1)
        gradient_score = gradient_for_next_layer.reshape(1, -1)

        self.gradient_weights[-2] += np.dot(self.received_inputs[-2].T, gradient_score)
        ...
        """
        gradient_for_next_layer = gradient_for_next_layer.reshape(self.input_image_shape)
